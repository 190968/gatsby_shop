        ...
        <DivInfo> 
            ...
            {gender.length === 1 ? gender : gender.length === 2 ? "BOYS and GIRLS" : ""}{" "}
             shoes and clothing <span>({props.order} products)</span>
                ...
                <Color color={props.size} onClick={()=>set_size(null)}>{props.size}</Color>
                <Color color={props.min} onClick={()=>set_min(0)}>{props.min}></Color>
                <Color color={props.max} onClick={()=>set_max(1000)}>{props.max}{"<"}</Color>
